...
domestic demand has stagnated, export markets have eroded, labor costs
continue to climb, and the margin for error is thin~\cite{gerlach2025usapple}.
...
woody tissue forms new cankers and, in severe cases, girdles and kills the
rootstock or trunk outright.
...
spatial resolution, conditions that a fixed, wide-area viewpoint cannot
consistently provide.
Moreover, the output a grower needs is not a binary classification of disease
presence but a spatial map of disease confidence across the tree that can
guide targeted pruning or treatment applications.
...
This approach enables the use of specialized sensors, namely flash RGB and
near-infrared cameras, that are highly effective for disease detection because
they resolve the fine bark texture characteristic of cankers, but that must be
operated at close range and in tight focus to do so, and consequently have
...
thesis aims to address that gap directly.
...
components: a multi-modal dataset and detector, a semantic mapping and
planning framework, and the field-validated robotic system that integrates
them.
...
We integrate these components into Erwin, a robotic active perception system
